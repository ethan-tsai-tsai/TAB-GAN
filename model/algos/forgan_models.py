...
class Generator(nn.Module):
    def __init__(self, noise_size, condition_size, generator_latent_size, cell_type, mean=0, std=1):
        ...
        if cell_type == "lstm":
            self.cond_to_latent = nn.LSTM(input_size=condition_size,
                                          hidden_size=generator_latent_size)
        else:
            self.cond_to_latent = nn.GRU(input_size=condition_size,
                                         hidden_size=generator_latent_size)
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(in_features=generator_latent_size + self.noise_size, out_features=9)

        )

    def forward(self, noise, condition):
        bs, seq_len, num_f = condition.shape
...
class Discriminator(nn.Module):
    ...
    def forward(self, prediction, condition):
        bs, seq_len, _ = condition.shape
        pred_len = prediction.shape[1]
        d_input = torch.cat((condition.view(bs, -1), prediction), dim=1)
        d_input = (d_input - self.mean) / self.std
        d_input = d_input.view(-1, self.condition_size * seq_len + pred_len, 1)
